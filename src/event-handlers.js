...
function handleReset() {
  ...
  // Remove both saved and auto-saved code from localStorage
...
export function setupEventListeners() {
  ...
  // Fix textarea resize functionality
  const textarea = document.getElementById('code');
  if (textarea) {
    // Stop propagation on resize handle interactions
    textarea.addEventListener('pointerdown', (e) => {
      // Check if the pointer is near the bottom-right corner (resize handle area)
      const rect = textarea.getBoundingClientRect();
      const handleSize = 20; // Approximate size of resize handle
